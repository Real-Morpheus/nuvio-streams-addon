...
async function curlRequest(url, options = {}) {
    return new Promise((resolve, reject) => {
        ...
        if (!headers['Accept']) headersCmd += ` -H "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8"`;
        if (!headers['Accept-Language']) headersCmd += ` -H "Accept-Language: en-US,en;q=0.9"`;
        ...
        const cmd = `curl -s -L --compressed --max-time 30 ${headersCmd}${proxyCmd} "${url}"`;
        console.log(`[HDHub4u] Curl: ${url}`);

        exec(cmd, { maxBuffer: 1024 * 1024 * 10 }, (error, stdout, stderr) => {
            if (error) {
                console.error(`[HDHub4u] Curl error: ${error.message}`);
                reject(error);
            } else {
                // Try to parse JSON
                try {
                    const json = JSON.parse(stdout);
                    resolve({ data: json, status: 200, headers: {} });
                } catch (e) {
                    resolve({ data: stdout, status: 200, headers: {} });
                }
            }
        });
    });
...
// Axios with retry helper (Wrapped around Curl)
async function axiosWithRetry(url, options = {}, retries = 3, backoff = 1000) {
    ...
    try {
        return await curlRequest(url, options);
    } catch (err) {
        if (retries > 1) {
...
function getRedirectLinks(url) {
    return axiosWithRetry(url, { headers: HEADERS })
        .then(response => {
        ...
        .then(doc => {
            ...
            if (wpHttp && data) {
                return axiosWithRetry(`${wpHttp}?re=${data}`, { headers: HEADERS })
...
function getDownloadLinks(mediaUrl) {
    return getCurrentDomain()
        .then(currentDomain => {
            ...
            return axiosWithRetry(mediaUrl, { headers: HEADERS });
        })
        .then(response => response.data) // response.data is text from curl
        .then(data => {
            const $ = cheerio.load(data);
